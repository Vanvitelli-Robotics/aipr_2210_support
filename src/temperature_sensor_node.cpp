...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("temp_sensor_sim");

  auto pub = node->create_publisher<sensor_msgs::msg::Temperature>("/temperature", 1);
  ...
  while (rclcpp::ok())
  {
    ...
    auto t = (node->now() - t0).seconds();

    auto cycle_t = std::fmod(t, 3 * TEMP_INTERVAL);

    double temp = N[0] * cos(w[0] * t) + N[1] * cos(w[1] * t) + N[2] * cos(w[2] * t);
